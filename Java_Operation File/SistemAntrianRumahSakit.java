...
public class SistemAntrianRumahSakit {
    private static final String DATA_FILE = "data_antrian.txt";
        ...
        while (true) {
            ...
            if (pilihan == 1) {
                    try {
                        ...
                        Pasien pasien = new Pasien(nama, prioritas, null );
                        antrian.add(pasien);
    ...
    private static Queue<Pasien> loadAntrianFromFile() {
        Queue<Pasien> antrian = new PriorityQueue<>();
    ...
    private static void simpanAntrianKeFile(Queue<Pasien> antrian) {
        try (FileWriter fileWriter = new FileWriter(DATA_FILE);
             BufferedWriter bufferedWriter = new BufferedWriter(fileWriter)) {
    
            for (Pasien pasien : antrian) {
             String data = String.format("%s,%d,%s\n", pasien.getNama(), pasien.getPrioritas(), pasien.getTanggalDaftar().toString());
